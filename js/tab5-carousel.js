...
在設計提示詞之前，你必須先在「心中背景思考過程中」根據 [原始文章] 整理一份「社群輪播圖規劃表」（Markdown 表格格式）。表格欄位與結構為：
...
- **【重要限制】此規劃表僅供你背景生成前的規劃參考，你絕對不需要在最終的輸出內容中包含這個表格！請直接從固定開頭文字開始輸出提示詞內容。**
...
5. **輸出格式**：
   - 在輸出的最前方，必須先輸出這行固定的開頭文字：「生成以下 4 張圖片，務必分開生成，一次只生一張，共 4 張」，換行後直接開始輸出各張圖片提示詞（**絕對不要在最終輸出中包含任何表格或規劃表**）。
   - 請**一次性**輸出完整的 4 張圖片提示詞，依序排版。
   - **不要**使用 any Markdown 程式碼區塊標記 (如 \`\`\`html 或 \`\`\`)。
   - 在每張圖片的開頭（例如：第 1 張圖片），必須精確輸出角色與變數對應表。
   - 輸出格式請嚴格符合以下範例，但所有內容必須替換為你針對當前 [原始文章] 所做出的生成結果，請勿抄寫範例中的 placeholder 括號字眼：

生成以下 4 張圖片，務必分開生成，一次只生一張，共 4 張

第 1 張圖片

[此處精確輸出對應的角色與變數對應表，例如：]
固定主持人 [姓名]：image1
固定小編 [姓名]：image2
本集來賓：[來賓名]：image3
...
提示詞：
請生成一張 1:1 正方形社群輪播封面圖，主題為《[圖片主標題]》。整體畫面\${exampleVisualTone}請將 image1 的固定主持人 [姓名]、image2 的固定小編 [姓名]、image3 海報中的本集來賓[來賓名]，設計成 \${exampleCharacterStyle}，所有人都要完整入鏡，不可裁切到臉、身體、手或腳。
